...
import { CLOUD, CLOUD, DESKTOP, DESKTOP, WEB, WEB } from './utils';

describe('utils', () => {
  describe('serialExecution', () => {
    test('rejects if no args', () => {
      return expect(utils.serialExecution()).rejects.toMatchObject(Error('Nothing to do'));
    });
  ...
  test('getUrlInfo', () => {
    ...
    expect(utils.getUrlInfo('http://neo:neoPass@guides.neo4j.com:1111/path?arg1=a&arg2=2')).toEqual(
      {
        protocol: 'http:',
  ...
  describe('addProtocolsToUrlList', () => {
    test('Add protocol where needed', () => {
      // Given
      const input = ['http://test.com', 'oskarhane.com', '*', null, 'https://mysite.com/guides'];
  ...
  test('stringifyMod works just as JSON.stringify with no modFn', () => {
    // Given
    const tests = [{ x: 1, y: ['yy', true, undefined, null] }, null, false, [[], [0]], 4];
  ...
  test('stringifyMod works just as JSON.stringify with modFn', () => {
    ...
    const expects = ['null', 'false', JSON.stringify([[], [0]]), '"4"', '4', '["string"]'];
    ...
    const tests = [
      ...
      [[], [0]],
      {
        ...
        prop2: [
          {
            innerProp: 'innerVal',
            innerProp2: [{ innerInner: 'innerVal2', innerInner2: 'innerInnerVal2' }]
          }
        ]
      },
      ['string']
...
describe('toKeyString', () => {
  ...
  describe('detectRuntimeEnv', () => {
    const tests: [any, any, any][] = [
      ...
      [
        'Cloud sub domain',
        [{ location: { href: 'https://xyxy.zyzy.cloud.com:7474/path' } }, ['cloud.com']],
        CLOUD
      ],
      [
        'No cloud match',
