...
export const hostIsAllowed = (uri: any, allowlist: any = null) => {
  ...
  const allowlistedHosts =
    allowlist && allowlist !== '' ? extractAllowlistFromConfigString(allowlist) : [];
  return (
    allowlistedHosts.indexOf(hostname) > -1 || allowlistedHosts.indexOf(hostnamePlusProtocol) > -1
  );
};
...
export const resolveAllowlistWildcard = (list: any, resolveTo: string[] = []) => {
...
export const stringifyMod = (
  ...
  const nextIndentation =
    nextPrettyLevel && !skipOpeningIndentation ? Array(nextPrettyLevel).join('  ') : '';
    ...
    if (typeof value.toJSON === 'function') {
      ...
      for (const k in value) {
        if (value.hasOwnProperty(k)) {
          tmp.push(
            `${nextIndentation}${JSON.stringify(k)}:${propSpacing}${stringifyMod(
              value[k],
              modFn,
              nextPrettyLevel,
              true
...
export const unescapeDoubleQuotesForDisplay = (str: any) => str.replace(/\\"/g, '"');
...
export const escapeReservedProps = (obj: any, prop: any) => {
  ...
  const localObj = safelyAddObjectProp(obj, getEscapedObjectProp(prop), obj[prop]);
...
const getUnescapedObjectProp = (prop: any) => (prop.indexOf('\\') === 0 ? prop.substr(1) : prop); // A bit weird because of escape chars
...
export function detectRuntimeEnv(win?: any, cloudDomains: string[] = []) {
  ...
  const parsedUrl =
    win && win.location && win.location.href ? parseUrl(win.location.href, true) : parseUrl('');
  if (parsedUrl.query.neo4jDesktopApiUrl && parsedUrl.query.neo4jDesktopGraphAppClientId) {
    return DESKTOP;
  }
