...
const nodeOutline = new Renderer({
  onGraphChange(selection: any, viz: any) {
    const circles = selection.selectAll('circle.outline').data((node: any) => [node]);
...
const nodeCaption = new Renderer({
  onGraphChange(selection: any, viz: any) {
    const text = selection.selectAll('text.caption').data((node: any) => node.caption);
    ...
    text
      ...
      .attr('font-size', (line: any) => viz.style.forNode(line.node).get('font-size'))
      .attr({
        fill(line: any) {
...
const nodeRing = new Renderer({
  onGraphChange(selection: any) {
    const circles = selection.selectAll('circle.ring').data((node: any) => [node]);
    circles
      .enter()
...
const arrowPath = new Renderer({
  ...
  onTick(selection: any) {
    return selection.selectAll('path').attr('d', (d: any) => d.arrow.outline(d.shortCaptionLength));
  }
});
...
const relationshipOverlay = new Renderer({
  ...
  onTick(selection: any) {
    ...
    return selection.selectAll('path.overlay').attr('d', (d: any) => d.arrow.overlay(band));
  }
});
