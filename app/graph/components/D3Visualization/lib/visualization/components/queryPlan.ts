...
function queryPlan(this: any, element: any) {
  ...
  const colors = d3.scale.ordinal().domain(d3.keys(operatorCategories)).range(operatorColors);
  ...
  const rows = function (operator: any) {
    let left;
    return (left = operator.Rows != null ? operator.Rows : operator.EstimatedRows) != null
      ? left
      : 0;
  };
  ...
  const operatorDetails = function (operator: any): any {
    ...
    if (
      ...
      wordWrap(identifiers.filter((d: any) => !/^ {2}/.test(d)).join(', '), 'identifiers');
  ...
  const layout = function (operators: any[], links: any[]) {
    const costHeight = (function () {
      ...
      return (operator: any) => scale((operator.DbHits != null ? operator.DbHits : 0) + 1);
    })();
    ...
    let width = d3.max(
      ranks.map((rank: any) => rank.values.length * (operatorWidth + operatorMargin))
    );
            ...
            (() => {
              ...
              for (operator of Array.from(rank.values)) {
                ...
                if (operator.children.length) {
                  const x =
                    d3.sum(operator.children, (child: any) => linkWidth(child) * center(child)) /
    ...
    width = d3.max(operators, (o: any) => o.x) - d3.min(operators, (o: any) => o.x) + operatorWidth;

    return [width, height];
  };

  const render = function (operators: any, links: any, width: any, height: any, redisplay: any) {
                ...
                data(d: any) {
                  ...
                  if (source.Rows != null || source.EstimatedRows != null) {
                    const [key, caption] = Array.from(
                      source.Rows != null ? ['Rows', 'row'] : ['EstimatedRows', 'estimated row']
                    );
                    return [
                      {
                        x,
        ...
        children: {
          '.operator': {
            ...
            selections(enter: any, update: any) {
              ...
              return update.transition().attr('transform', (d: any) => `translate(${d.x},${d.y})`);
            },
            children: {
              'rect.background': {
                data(d: any) {
                              ...
                              ? operatorCornerRadius -
                                Math.sqrt(
                                  Math.pow(operatorCornerRadius, 2) -
                                    Math.pow(
                                      operatorCornerRadius - d.height + operatorHeaderHeight,
                                      2
                                    )
                                )
                              : 0;
                  ...
                  'path.expand': {
                    ...
                    selections(enter: any, update: any) {
                      ...
                      enter
                        .append('path')
                        .attr('class', 'expand')
                        .attr('fill', (d: any) => color(d.operatorType)['text-color-internal'])
              ...
              'g.detail': {
                ...
                selections(enter: any, update: any, exit: any) {
                  ...
                  update
                    ...
                    .attr('transform', (d: any) => `translate(0, ${operatorHeaderHeight + d.y})`)
              ...
              'rect.outline': {
                ...
                selections(enter: any, update: any) {
                  ...
                  return update
                    ...
                    .attr('fill', 'none')
                    .attr('stroke-width', 1)
                    .style('stroke', (d: any) => color(d.operatorType)['border-color']);
                }
              }
            }
          }
