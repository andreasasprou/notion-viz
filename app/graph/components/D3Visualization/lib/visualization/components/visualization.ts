...
const vizFn = function (el: any, measureSize: any, graph: any, layout: any, style: any) {
  const viz: any = { style };
  ...
  const zoomBehavior = d3.behavior.zoom().scaleExtent([0.2, 1]).on('zoom', zoomed);
  ...
  const interpolateZoom = (translate: any, scale: any) =>
    d3
      ...
      .duration(500)
      .tween('zoom', () => {
        const t = d3.interpolate<number, number>(zoomBehavior.translate(), translate);
  ...
  const newStatsBucket = function () {
    ...
    bucket.fps = () => ((1000 * bucket.frameCount) / bucket.duration()).toFixed(1);
    bucket.lps = () => ((1000 * bucket.layout.layoutSteps) / bucket.duration()).toFixed(1);
    bucket.top = function () {
      let time;
      const renderers = [];
      ...
      const totalRenderTime = renderers.reduce((prev, current) => prev + current.time, 0);
      return renderers
        .map((d) => `${d.name}: ${((100 * d.time) / totalRenderTime).toFixed(1)}%`)
        .join(', ');
    };
    return bucket;
  };
  ...
  const render = function () {
    ...
    const relationshipGroups = container
      ...
      .attr(
        'transform',
        (d: any) => `translate(${d.source.x} ${d.source.y}) rotate(${d.naturalAngle + 180})`
      );
  ...
  viz.update = function () {
    ...
    const layers = container.selectAll('g.layer').data(['relationships', 'nodes']);
    layers
      .enter()
      .append('g')
    ...
    relationshipGroups.classed('selected', (relationship: any) => relationship.selected);
  ...
  viz.resize = function () {
    ...
    return root.attr(
      'viewBox',
      [0, (layoutDimension - size.height) / 2, layoutDimension, size.height].join(' ')
    );
  };
