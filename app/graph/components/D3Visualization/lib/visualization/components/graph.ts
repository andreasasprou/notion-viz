...
export default class Graph {
    ...
    for (const eNode of Array.from(nodes)) {
      if (this.findNode(eNode.id) == null) {
        ...
        this.expandedNodeMap[node.id] = this.expandedNodeMap[node.id]
          ? [...(new Set(this.expandedNodeMap[node.id].concat([eNode.id])) as any)]
          : [eNode.id];
      }
    }
  };
  ...
  pruneInternalRelationships() {
    const relationships = this._relationships.filter((relationship: any) => !relationship.internal);
    this.relationshipMap = {};
    this._relationships = [];
  ...
  findNodeNeighbourIds(id: any) {
    return this._relationships
      .filter((relationship: any) => relationship.source.id === id || relationship.target.id === id)
