...
export class ForceGraph extends React.Component<ForceGraphProps, ForceGraphState> {
  focusAnimationTimeout?: NodeJS.Timeout;
  ...
  _onDragMove = (ev, index, nodeList) => {
    ...
    if (!this.state.config.staticGraph) {
      ...
      const shouldUpdateNode =
        !this.state.config.bounded || isPositionInBounds({ x: newX, y: newY }, this.state);
  ...
  _zoomed = () => {
    ...
    if (
      this.debouncedOnZoomChange &&
      this.state.previousZoom !== transform.k &&
      !this.state.config.panAndZoom
    ) {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    const { graphElementsUpdated, newGraphElements } = checkForGraphElementsChanges(
      nextProps,
      this.state
    );
