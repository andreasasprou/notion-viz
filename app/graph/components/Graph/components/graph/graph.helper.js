...
function checkForGraphConfigChanges(nextProps, currentState) {
  ...
  const configUpdated =
    newConfig && !isEmptyObject(newConfig) && !isDeepEqual(newConfig, currentState.config);
  const d3ConfigUpdated =
    newConfig && newConfig.d3 && !isDeepEqual(newConfig.d3, currentState.config.d3);
...
function getNormalizedNodeCoordinates(
  ...
  switch (targetSymbolType) {
    case CONST.SYMBOLS.CIRCLE: {
      ...
      let strokeSize =
        strokeWidth * Math.min(config.link?.markerWidth || 0, config.link?.markerHeight || 0);
